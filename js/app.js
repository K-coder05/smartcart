...
let currentList = []; // {name, category, quantityInStore, costInStore, qty, included}
							...
							properties: {
								...
								ingredients: {
									...
									items: {
										...
										required: ["name", "category", "quantityInRecipe", "costInRecipe", "quantityInStore", "costInStore"]
									}
								}
							},
							required: ["name", "time", "costPerServing", "totalCostInStore", "instructions", "ingredients"]
						}
					}
				}
			});
...
document.addEventListener("DOMContentLoaded", () => {
	...
	// generic data-target navigation (back buttons, nav items, in-sheet links)
	...
	// OAuth buttons — not wired to a real provider in this demo
	...
	const drawMatchList = () => {
		...
		list.forEach((recipe) => {
			...
			const bg = hashPick(recipe.name, THUMB_BG);
			const emoji = hashPick(recipe.name, THUMB_EMOJI);
			const isSaved = savedNames.has(recipe.name);
			div.innerHTML = `
				<div class="match-card__thumb" style="background:${bg};">${emoji}</div>
